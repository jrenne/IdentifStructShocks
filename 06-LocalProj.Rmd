...
\end{equation}
Running that kind of regression (using instruments for $\eta_{j,t}$) is the core idea of the **local projection approach** proposed by @Jorda_2005.

Now, how to proceed in the (usual) case where $\eta_{j,t}$ is not observed? We consider two situations. While the second requires some instruments, the first approach does not; it corresponds to the original @Jorda_2005's approach. 
...
The covariance matrix of $\bv{b}_{iv}$ can be approximated by $\frac{1}{T}Q(\bv{X},\bv{Z})\hat{\bv{S}}Q(\bv{X},\bv{Z})'$ where $\hat{\bv{S}}$ is the Newey-West estimator of $\bv{S}$.^[That is: $\hat{\bv{S}}=\hat\gamma_0 + 2 \sum_{\nu=1}^{q}\left(1-\frac{\nu}{q+1}\right) \hat\gamma_\nu$, where the $\hat\gamma_j$'s are sample auto-covariances of $\bv{z}_t\varepsilon_t$, see @Newey_West_1987]
