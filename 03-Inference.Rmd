...
res.svar.ordering <- svar.ordering.2(y,p=3,
                                   posit.of.shock = 5,
                                   nb.periods.IRF = 20,
                                   inference = 3,# 0 -> no inference, 1 -> parametric bootstrap, 2 <- non-parametric bootstrap, 3 <- monte carlo, 4 <- bootstrap-after-bootstrap
                                   nb.draws = 200,
...
res.svar.ordering <- svar.ordering.2(y,p=3,
                                   posit.of.shock = 5,
                                   nb.periods.IRF = 20,
                                   inference = 2,# 0 -> no inference, 1 -> parametric bootstrap, 2 <- non-parametric bootstrap, 3 <- monte carlo, 4 <- bootstrap-after-bootstrap
                                   nb.draws = 200,
                                   confidence.interval = 0.90, # expressed in pp.
                                   indic.plot = 1 # Plots are displayed if = 1.
)
...
# Distribution of coefficients stemming from the non-parametric bootstrap
h <- 5
par(mfrow=c(2,ifelse(round(n/2)==n/2,n/2,(n+1)/2)))
for (i in 1:n){
hist(simulated.IRFs.ordering.bootstrap[h,i,],xlab="",ylab="",
     main=paste("Effect of shock at horizon ",h,"on ",considered.variables[i],sep=""))
lines(array(c(IRFs.ordering.bootstrap[h,i],IRFs.ordering.bootstrap[h,i],0,100),c(2,2)),col="red")
lines(array(c(median.IRFs.ordering.bootstrap[h,i],median.IRFs.ordering.bootstrap[h,i],0,100),c(2,2)),col="blue")
text(IRFs.ordering.bootstrap[h,i],25,label="Estimated coefficient",col="red")
}
```
...
res.svar.small.sample <- svar.ordering.2(small.sample,p=3,
                                         posit.of.shock = 5,
                                         nb.periods.IRF = 20,
                                         inference = 2,# 0 -> no inference, 1 -> parametric bootstrap, 2 <- non-parametric bootstrap, 3 <- monte carlo
                                         nb.draws = 200,
                                         confidence.interval = 0.90, # expressed in pp.
                                         indic.plot = 1 # Plots are displayed if = 1.
)
...
for (i in 1:n){
  hist(simulated.IRFs.small.sample[h,i,],xlab="",ylab="",
       main=paste("Effect of shock at horizon ",h,"on ",considered.variables[i],sep=""))
  lines(array(c(IRFs.small.sample[h,i],IRFs.small.sample[h,i],0,100),c(2,2)),col="red")
  lines(array(c(median.IRFs.small.sample[h,i],median.IRFs.small.sample[h,i],0,100),c(2,2)),col="blue")
  lines(array(c(IRFs.ordering.bootstrap[h,i],IRFs.ordering.bootstrap[h,i],0,100),c(2,2)),col="black")
  text(IRFs.small.sample[h,i],25,label="Estimated coefficient",col="red")
  text(IRFs.ordering.bootstrap[h,i],30,label="True coefficient",col="black")
}
```
...
res.svar.ordering <- svar.ordering.2(y,p=3,
                                   posit.of.shock = 5,
                                   nb.periods.IRF = 20,
                                   inference = 4,# 0 -> no inference, 1 -> parametric bootstrap, 2 <- non-parametric bootstrap, 3 <- monte carlo, 4 <- bootstrap-after-bootstrap
                                   nb.draws = 200,
                                   confidence.interval = 0.90, # expressed in pp.
                                   indic.plot = 1 # Plots are displayed if = 1.
)
