...
The restrictions $B'B = Id$ can be eliminated by parameterizing $B$ in such a way that, whatever the consider parameters, $B$ is orthogonal.^[@Jarocinski_2021 develops a ML approach that does not necessitates to parameterize the space of orthogonal matrices as he does not proceed under the assumption that $B'B$ is orthogonal.] @Gourieroux_Monfort_Renne_2017 propose to use, for that, the Cayley's representation: any orthogonal matrix with no eigenvalue equal to $-1$ can be written as
\begin{equation}
B(A) = (Id+A) (Id-A)^{-1},
\end{equation}
