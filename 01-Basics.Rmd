...
\]
Hence, the **impulse response function (IRF)** at horizon $h$ is simply $\Psi_h = \phi^h b $.

```{r simAR1, fig.align = 'left-aligned', out.width = "90%", fig.cap = "Impulse response functions for an AR(1) process: $y_t = \\phi y_{t-1} + b \\eta_t$, $\\eta_t \\sim i.i.d. \\mathcal{N}(0,1)$."}
phi <- .8
b <- .5
...
* Calibrate and solve a (purely structural) Dynamic Stochastic General Equilibrium (DSGE) model at first order (linear approximation). The resulting solution can be written in the form of Eq. \@ref(eq:InfMA).

* Directly estimate the $\Psi_h$ matrices using **projection methods** (see Section \@ref(Projections)).

* Approximate the infinite moving-average representation by estimating a parsimonious model, such as a **VAR(MA)** model (see Section \@ref(estimVAR)). Once a (structural) VARMA representation is obtained, Eq. \@ref(eq:InfMA) follows immediately from the result stated in the next proposition.
...
$$
with $\Phi(L) = \Phi_1 + \Phi_2 L + \dots + \Phi_p L^{p-1}$, and where the reduced-form shocks ($\varepsilon_{t}$) are white noise shocks.^[That is, these shocks are such that $\mathbb{E}(\varepsilon_{t})=0$, and $\mathbb{C}ov(\varepsilon_{t},\varepsilon_{t-k})=\Omega$ if $k=0$, and a zero matrix if $k \ne 0$.]
...
### Maximum Likelihood Estimation (MLE) when the shocks are Gaussian
...
### Asymptotic distribution of the OLS estimates
...
However, as stated by Proposition \@ref(prp:OLSVAR) when $y_t$ is covariance stationary, then $\frac{1}{n}\bv{X}'\bv{X}$ converges to a positive definite matrix $\bv{Q}$, and $\frac{1}{n}X'\boldsymbol\varepsilon_i$ converges to 0. Hence $\bv{b}_i \overset{p}{\rightarrow} \beta_i$.
...
In practice, to use the previous proposition (for instance to implement Monte-Carlo simulations), $\Omega$ is replaced with $\hat{\Omega}$, $\bv{Q}$ is replaced with $\hat{\bv{Q}} = \frac{1}{T}\sum_{t=p}^T x_t x_t'$ and $\Sigma$ with the matrix whose components are of the form $\hat\sigma_{i,l}\hat\sigma_{j,m}+\hat\sigma_{i,m}\hat\sigma_{j,l}$, where the $\hat\sigma_{i,l}$'s are the components of $\hat\Omega$.
...
test.pp.Y2   <- pp.test(Y[,2])$p.value
test.kpss.Y1 <- kpss.test(Y[,1])$p.value
test.kpss.Y2 <- kpss.test(Y[,2])$p.value
test.adf  <- c(test.adf.Y1,test.adf.Y2)
test.pp   <- c(test.pp.Y1,test.pp.Y2)
test.kpss <- c(test.kpss.Y1,test.kpss.Y2)
rbind(test.adf,test.pp,test.kpss)
```
...
Check for residual autocorrelation (Portmanteau test) using the Portmanteau test or the one developed by @Breusch_1978 and @Godfrey_1978 (a high p-value indicates no evidence of serial correlation):
...
Let us now test for heteroskedasticity that would take place in the form of ARCH (see @Engle_1982). For that, we ca use the `arch.test` function (a high p-value suggests no ARCH effects):

```{r}
arch.test(estVAR, lags.multi = 12, multivariate.only = TRUE)
```

Normality test can be perform using function `normality.test` (a high p-value suggests residuals are consistent with normality):

```{r}
normality.test(estVAR)
```
:::
...
grangertest(x=as.matrix(US3var[,c("y.gdp.gap","infl")]),order=3)
grangertest(x=as.matrix(US3var[,c("infl","y.gdp.gap")]),order=3)
```
