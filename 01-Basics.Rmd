...
If the dynamics of process $y_t$ can be described as a VARMA model, and if $y_t$ is covariance stationary (see Def. \@ref(def:covstat)), then $y_t$ admits the following infinite MA representation (MA($\infty$)):^[This is also the Wold decomposition of process $\{y_t\}$.]
\begin{equation}
...
To select appropriate lag length, **selection criteria** can be used.  In the context of VAR models, using Eq. \@ref(eq:optimzedLogL), we have for instance:
\begin{eqnarray*}
